...
//    public var isBeta:String?
    ...
    public required init(status:String, getCustomerAC:Bool?) { //getFlightList
        super.init()
        self.status = status
        self.getCustomerAC = getCustomerAC
    }
...
//        if let value = isBeta { dictionary[SerializationKeys.isBeta] = value }
        ...
        if let value = getCustomerAC { dictionary[SerializationKeys.getCustomerAC] = value }
        return dictionary
    }
}
...
extension SmallRequest {
    private struct SerializationKeys {
        ...
        static let getCustomerAC = "customerAc"
    }
}
